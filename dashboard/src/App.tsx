...
export function App({ api = localAPI }: { api?: DashboardAPI }) {
  ...
  const refresh = () => {
    ...
    Promise.all([api.loadSummary(), api.loadSnapshot(), api.loadPrivacy()])
      ...
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Local API unavailable'));
  };
  useEffect(refresh, [api]);

  const inspectSession = (id: string) => api.loadSession(id).then((result) => setEvents(result.events)).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Evidence unavailable'));
  const savePrivacy = () => api.updatePrivacy(privacy).then((next) => { setPrivacy(next); setSaved(true); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Settings unavailable'));
    ...
    <main className="doctor-shell">
      ...
      <div className="doctor-layout">
        ...
        <section className="doctor-stage" aria-live="polite" key={`${route}-${locale}`}>
          ...
          <Page route={route} locale={locale} summary={summary} snapshot={snapshot} privacy={privacy} setPrivacy={setPrivacy} events={events} inspectSession={inspectSession} savePrivacy={savePrivacy} saved={saved} refresh={refresh} />
        </section>
      </div>
...
type PageProps = { route: Route; locale: Locale; summary: Summary; snapshot: Snapshot; privacy: PrivacySettings; setPrivacy(value: PrivacySettings): void; events: SafeEvent[]; inspectSession(id: string): void; savePrivacy(): void; saved: boolean; refresh(): void };
function Page(props: PageProps) {
  const { route, locale, summary, snapshot } = props;
  ...
  if (route === 'task') return <div className="data-stack">{snapshot.sessions.length === 0 ? <Empty text={locale === 'en' ? 'No sessions recorded yet.' : '尚无会话记录。'} /> : snapshot.sessions.map((session) => <article className="data-row" key={session.id}><div><strong>{session.id}</strong><span>{session.client} · {session.model} · {session.eventCount} events</span></div><button onClick={() => props.inspectSession(session.id)} aria-label={`Inspect ${session.id}`}>Inspect ↗</button></article>)}{props.events.map((event) => <article className="timeline-row" key={event.eventId}><strong>{event.eventType}</strong><span>{event.precision} · {event.provenance}</span><time>{event.timestamp}</time></article>)}</div>;
  ...
  if (route === 'comparison') return snapshot.comparisonCount === 0 ? <Empty text={locale === 'en' ? 'No matched comparison yet. Collect at least 15 comparable tasks per cohort.' : '尚无匹配对比。每个样本组至少需要 15 个可比任务。'} /> : <Metric value={snapshot.comparisonCount} label={locale === 'en' ? 'saved comparisons' : '已保存对比'} />;
  if (route === 'trends') return <div className="data-stack">{snapshot.trends.length === 0 ? <Empty text={locale === 'en' ? 'No baseline yet.' : '尚未形成基线。'} /> : snapshot.trends.map((point) => <article className="trend-row" key={point.date}><time>{point.date}</time><div><span style={{ width: `${Math.min(100, point.events * 5)}%` }} /></div><strong>{point.sessions} sessions · {point.events} events</strong></article>)}</div>;
  ...
  return <form className="privacy-form" onSubmit={(event) => { event.preventDefault(); props.savePrivacy(); }}><label><input type="checkbox" checked={props.privacy.capturePrompts} onChange={(event) => props.setPrivacy({ ...props.privacy, capturePrompts: event.target.checked })} /> Capture prompts</label><label><input type="checkbox" checked={props.privacy.captureFileContents} onChange={(event) => props.setPrivacy({ ...props.privacy, captureFileContents: event.target.checked })} /> Capture file contents</label><label>Retention days<input aria-label="Retention days" type="number" min="1" max="3650" value={props.privacy.retentionDays} onChange={(event) => props.setPrivacy({ ...props.privacy, retentionDays: Number(event.target.value) })} /></label><button className="action-button" type="submit">Save privacy settings</button>{props.saved && <span role="status">Saved locally.</span>}</form>;
}
...
function Empty({ text }: { text: string }) { return <div className="empty-state"><span>○</span><strong>{text}</strong><p>Unavailable data is never shown as zero.</p></div>; }
