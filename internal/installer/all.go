...
func BuildAllPlan(home string) (Plan, error) {
	...
	codexSkill := []byte("---\nname: agent-doctor\ndescription: Use local runtime evidence to keep coding tasks from looping, losing context, or finishing without validation.\n---\n\nAt the start of every coding turn, call the Agent Doctor MCP get_runtime_guidance tool with projectId set to the current project ID (normally the current working directory). When starting or resuming work in a known project, also call get_context_capsule so a bounded cross-client task handoff and confirmed project memory are available. Call guidance again after a failed tool step, context compaction, and before completion. Follow evidence-backed advise, redirect, block, or verification instructions before continuing. When non-continue guidance changes the work, briefly tell the user `Agent Doctor intervened: <action taken>`. Treat continue as silence. Codex MCP guidance cannot force-block tool calls; never claim it did.\n")
	agents := AppendMarkedBlock(before[2], codexGuidanceOwner, []byte("Use the agent-doctor skill during coding tasks. At the start of every coding turn call get_runtime_guidance with projectId set to the current project ID. When starting or resuming a known project, also call get_context_capsule to load the bounded cross-client task handoff and confirmed project memory. Call guidance again after a failed tool step, context compaction, and before completion. Follow non-continue guidance before continuing and state `Agent Doctor intervened: <action taken>` when it changes the work. Continue results stay silent. Codex MCP advice cannot force-block tool calls; run requested deterministic verification before claiming completion."))
	...
	claudeSkill := []byte("---\nname: agent-doctor\ndescription: Follow local evidence-backed reliability guidance while using Claude Code.\n---\n\nAt SessionStart, Agent Doctor automatically injects a bounded cross-client task handoff for the same project when one is available. It contains confirmed project memory and the latest captured task state with provenance, not a full transcript. Agent Doctor hooks otherwise observe content-free evidence fingerprints. Respect redirect and verification guidance. In Guard or Auto Guard, supported PreToolUse and Stop decisions may be blocked; other guidance remains advisory.\n")
